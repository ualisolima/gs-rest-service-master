...
const fetchPublisherList = () => (dispatch, getState) => {
...
const saveEditingItem = item => (dispatch, getState) => {
    ...
    if (getState.isEditing && item.name != '' && item.location != '') {
        fetch('http://localhost:8080/publishers/' + item.id, {
            mode: 'cors',
...
const loadPublications = item => (dispatch, getState) => {
    ...
    fetch('http://localhost:8080/publishers/'+item.id+'/pubs', {
        mode: 'cors',
        headers: { 'Content-Type': 'application/json' },
    })
        .then(response => response.json())
